...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  int _orderNumber = 1;
  static const String defaultCashierPrinterName = 'Printer001-1E3B';
  static const String defaultKitchenPrinterName = '4B-2023PA-3319';
  static const String defaultBotPrinterName = '4B-2023PA-EE15';
...
Future<void> _saveInvoice({bool isDue = false}) async {
  if (_cartItems.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text('Cart is empty. Add items before saving.'),
        backgroundColor: Colors.red,
        behavior: SnackBarBehavior.floating,
      ),
    );
  ...
  // For NEW orders, require printers to be connected
  
  final hasKitchenItems = _hasKitchenItems(_cartItems);
  bool canSave = true;
  String errorMessage = '';
  
  // ALWAYS require cashier printer for any invoice
  if (_connectedCashierDevices.isEmpty) {
    canSave = false;
    errorMessage = 'Cannot save invoice: Cashier printer not connected. Please connect to a cashier printer first.';
  }
  
  // Require kitchen printer if there are kitchen items
  if (hasKitchenItems && _connectedKitchenDevices.isEmpty) {
    canSave = false;
    errorMessage = 'Cannot save invoice: Kitchen printer not connected. Please connect to a kitchen printer to print KOT.';
  }
  
  // Require BOT printer for any items
  if (_cartItems.isNotEmpty && _connectedBotDevices.isEmpty) {
    canSave = false;
    errorMessage = 'Cannot save invoice: BOT printer not connected. Please connect to a BOT printer.';
  }
  ...
  try {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      // Print receipts (printers are confirmed to be connected)
      if (_connectedKitchenDevices.isNotEmpty && hasKitchenItemsInCart) {
        await _printKOT(onlyNewItems: false);
      }

      if (_connectedBotDevices.isNotEmpty && cartItemsToSave.isNotEmpty) {
        await _printKOT(onlyNewItems: false, printBOT: true);
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Invoice saved successfully${invoiceNumber != null ? ' (#$invoiceNumber)' : ''}'),
...
Future<void> _updateDueTableInvoice() async {
  if (_cartItems.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text('Cart is empty. No items to update.'),
        backgroundColor: Colors.red,
        behavior: SnackBarBehavior.floating,
      ),
    );
  ...
  // ============ CHECK PRINTER CONNECTIVITY FOR DUE TABLE UPDATES ============
  // For due table updates, also require printers to be connected
  ...
  if (_connectedCashierDevices.isEmpty) {
    canUpdate = false;
    errorMessage = 'Cannot update due table: Cashier printer not connected. Please connect to a cashier printer first.';
  }
  
  // Require kitchen printer if there are NEW kitchen items
  if (hasNewKitchenItems && _connectedKitchenDevices.isEmpty) {
    canUpdate = false;
    errorMessage = 'Cannot update due table: Kitchen printer not connected. Please connect to a kitchen printer to print KOT for new items.';
  }
  
  // Require BOT printer if there are NEW items
  if (newKitchenItems.isNotEmpty && _connectedBotDevices.isEmpty) {
    canUpdate = false;
    errorMessage = 'Cannot update due table: BOT printer not connected. Please connect to a BOT printer to print new items.';
  }
  ...
  try {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Due table updated successfully${invoiceNumber != null ? ' (#$invoiceNumber)' : ''}'),
          backgroundColor: Colors.green,
          behavior: SnackBarBehavior.floating,
        ),
      );
...
class _PaymentScreenState extends State<PaymentScreen>
    ...
    if (!_tabController.indexIsChanging) {
      ...
      switch (currentIndex) {
        ...
        case 3: // Card
          ...
          // ALWAYS update card amount when card tab is selected, for both regular and due tables
          _updateCardAmountBasedOnRemainingBalance();
          break;
      }
    }
  }

  void _updateCardAmountBasedOnRemainingBalance() {
    ...
    setState(() {
      if (currentRemainingBalance > 0) {
        ...
        // No remaining balance, set card to 0
        cardPaid = 0.0;
  ...
  void _updatePaymentAmount(String method, String value) {
    ...
    // For non-cash methods, limit to remaining balance
    if (method != 'Cash' && amount > maxAllowed) {
      amount = maxAllowed > 0 ? maxAllowed : 0;

      // Update controller with the limited amount
      WidgetsBinding.instance.addPostFrameCallback((_) {
        switch (method) {
          case 'Bank Transfer':
            _bankTransferAmountController.text =
    ...
    // For Cash method, allow any amount (can be overpayment)
    ...
    // If we're currently on the card tab, update the card amount to reflect changes
  ...
  Future<Map<String, dynamic>> _processPayment() async {
    ...
    // Check if total payment is less than invoice amount
    if (totalPaid < widget.netAmount) {
      _showError(
          'Total payment (${_fmt(totalPaid)}) is less than invoice amount (${_fmt(widget.netAmount)})');
      setState(() => _isProcessingPayment = false);
      return {'success': false, 'invoiceNumber': null};
    }
                            ...
                            TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                helperText: widget.isDueTable
                                    ? 'Enter cash amount - card will auto-adjust'
                                    : 'Enter cash amount',
                          ...
                          children: [
                            ...
                            TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                helperText: widget.isDueTable
                                    ? 'Enter bank transfer amount - card will auto-adjust'
                                ...
                                children: [
                                  ...
                                  TextField(
                                    ...
                                    decoration: InputDecoration(
                                      ...
                                      helperText: widget.isDueTable
                                          ? 'Enter credit amount - card will auto-adjust'
                                          : 'Enter credit amount',
                                      helperStyle: TextStyle(
                                        color: primaryColor,
                                        fontSize: 12,
                          ...
                          children: [
                            ...
                            TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                helperText: 'Auto-adjusted based on other payment methods',
                                helperStyle: TextStyle(
                                  color: primaryColor,
                                  fontSize: 12,
                                ),
                        ...
                        children: [
                          ...
                          _buildPaymentSummaryRow(
                              ...
                              color: remainingBalance < 0 
                                  ? Colors.green 
                                  : (remainingBalance > 0 ? Colors.red : primaryColor)),
                        ],
                      ),
                    ),
                  ),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildPaymentBreakdownRow('TOTAL:', totalPaid,
                              isTotal: true),
                          if (remainingBalance < 0)
                            _buildPaymentBreakdownRow('CHANGE:', -remainingBalance,
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      SizedBox(
                        width: MediaQuery.of(context).size.width * 0.18,
                        child: ElevatedButton(
                          onPressed: _isProcessingPayment || totalPaid < widget.netAmount
                              ? null
                              : () async {
                          ...
                          style: ElevatedButton.styleFrom(
                            backgroundColor: _isProcessingPayment || totalPaid < widget.netAmount
                                ? Colors.grey
                                : accentColor,
                            foregroundColor: Colors.black87,
                            shape: RoundedRectangleBorder(
      ...
      children: [
        ...
        Text(
          ...
          style: GoogleFonts.poppins(
            fontSize: 16,
            fontWeight: isBold ? FontWeight.w600 : FontWeight.normal,
            color: color ?? (isNegative ? Colors.red : primaryColor),
          ),
        ),
      ],
    );
        ...
        children: [
          Text(
            ...
            style: GoogleFonts.poppins(
              ...
              color: color ?? (isTotal ? primaryColor : Colors.black87),
            ),
          ),
          Text(
            _fmt(amount),
